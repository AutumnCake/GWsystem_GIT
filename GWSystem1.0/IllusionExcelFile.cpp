...
//  
CApplication0 IllusionExcelFile::excel_application_;

IllusionExcelFile::IllusionExcelFile() :already_preload_(FALSE)
{
}
...
{
	CloseExcelFile();
	ReleaseExcel();//此处除了释放Excel文件外还要释放OLE，否则在关闭程序时会报错
}
...
//打开excel文件  
BOOL IllusionExcelFile::OpenExcelFile(CString file_name)
{
	//先关闭  
	CloseExcelFile();
...
BOOL IllusionExcelFile::LoadSheet(CString sheet, BOOL pre_load)
{
...
void IllusionExcelFile::SetCellString(long irow, long icolumn, CString new_string)
{
	...
	CRange0 start_range = excel_work_sheet_.get_Range(COleVariant(_T("A1")), covOptional);
	CRange0 write_range = start_range.get_Offset(COleVariant((long)irow - 1), COleVariant((long)icolumn - 1));
